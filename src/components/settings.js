  ...
  function copyOccurrenceOverridesToYear(entries, fromOvs, fromYear, toYear, existingToOvs = {}) {
    ...
    Object.keys(fromOvs || {}).forEach((key) => {
      ...
      if (!valid.has(newKey) || existingToOvs[newKey]) return;
      const copy = {};
      ["desc", "amount", "notes", "month", "day"].forEach((f) => {
        if (fromOvs[key][f] !== void 0) copy[f] = fromOvs[key][f];
      });
      if (Object.keys(copy).length) added[newKey] = copy;
    });
    return added;
  }
  ...
  // differs from what the entry would produce anyway, and occurrences that
  // already have an override (existing or just-copied) are not touched.
