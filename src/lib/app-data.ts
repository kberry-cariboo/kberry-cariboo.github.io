...
import type { Account, Member } from "../types.js";
  export interface CategoriesContextValue {
    categories: string[];
    categoryColors: Record<string, string>;
    /** The card background chips are drawn against, for contrast. */
    chipSurface?: string;
  }
  export const CategoriesContext = createContext<CategoriesContextValue>({ categories: [], categoryColors: {} });
  ...
  // canWrite defaults true: a component reading this before the provider is in
  ...
  /** The signed-in member, as App derives it from the session and member list. */
  ...
  export interface HouseholdContextValue {
    ...
    logActivity: (kind: string, what: string) => void;
  }
  export const HouseholdContext = createContext<HouseholdContextValue>({ members: [], sessionUser: null, accounts: [], canWrite: true, myRole: null, logActivity: () => {
  } });
...
export function lowBalanceEpisodes(flow, alertThreshold, horizonDays = RUNWAY_DAYS) {
  ...
  return episodes.map((e) => ({
    ...
    days: e.recover
      ? Math.max(1, Math.round((e.recover.date.getTime() - e.start.date.getTime()) / 864e5))
      : Math.max(1, Math.round((end.getTime() - e.start.date.getTime()) / 864e5)),
    openEnded: !e.recover
  }));
}
